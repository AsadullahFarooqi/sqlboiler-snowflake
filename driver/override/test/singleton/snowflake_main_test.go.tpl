var rgxSnowflakekey = regexp.MustCompile(`(?m)((,\n)?\s+CONSTRAINT.*?FOREIGN KEY.*?\n)+`)
...
func (m *mysqlTester) setup() error {
	...
	viper.SetDefault("snowflake.sslmode", "true")
	viper.SetDefault("snowflake.port", 3306)

	m.dbName = viper.GetString("snowflake.dbname")
	m.host = viper.GetString("snowflake.host")
	m.user = viper.GetString("snowflake.user")
	m.pass = viper.GetString("snowflake.pass")
	m.port = viper.GetInt("snowflake.port")
	m.sslmode = viper.GetString("snowflake.sslmode")
	m.testDBName = viper.GetString("snowflake.testdbname")
	m.skipSQLCmd = viper.GetBool("snowflake.skipsqlcmd")

	err = vala.BeginValidation().Validate(
		vala.StringNotEmpty(m.user, "snowflake.user"),
		vala.StringNotEmpty(m.host, "snowflake.host"),
		vala.Not(vala.Equals(m.port, 0, "snowflake.port")),
		vala.StringNotEmpty(m.dbName, "snowflake.dbname"),
		vala.StringNotEmpty(m.sslmode, "snowflake.sslmode"),
	).Check()
	...
	if !m.skipSQLCmd {
		...
		dumpCmd := exec.Command("snowflakedump", m.defaultsFile(), "--no-data", m.dbName)
		createCmd := exec.Command("snowflake", m.defaultsFile(), "--database", m.testDBName)
		...
		createCmd.Stdin = newFKeyDestroyer(rgxSnowflakekey, r)
		...
		if err = createCmd.Start(); err != nil {
			return errors.Wrap(err, "failed to start snowflake command")
		}
		...
		if err = createCmd.Wait(); err != nil {
			...
			return errors.Wrap(err, "failed to wait for snowflake command")
		}
	}
...
func (m *mysqlTester) conn() (*sql.DB, error) {
	...
	var err error
	m.dbConn, err = sql.Open("snowflake", driver.SnowflakeBuildQueryString(m.user, m.pass, m.testDBName, m.host, m.port, m.sslmode))
	if err != nil {
	return nil, err
	}
