...
import (
	...
	"github.com/asadullahfarooqi/sqlboiler/v4/importers"
)
...
func (m *SnowflakeDriver) Assemble(config drivers.Config) (dbinfo *drivers.DBInfo, err error) {
	...
	m.connStr = SnowflakeBuildQueryString(user, pass, dbname, host, port, sslmode)
...
// SnowflakeBuildQueryString builds a query string for Snowflake.
func SnowflakeBuildQueryString(user, pass, dbname, host string, port int, sslmode string) string {
	...
	// Snowflake is a bad, and by default reads date/datetime into a []byte
...
func (m *SnowflakeDriver) foreignKeyInfoFromDB(schema, tableName string) ([]drivers.ForeignKey, error) {
...
// as a Column object.
// Deprecated: for Snowflake enum types to be created properly TranslateTableColumnType method should be used instead.
...
func (SnowflakeDriver) Imports() (col importers.Collection, err error) {
	...
	col.TestSingleton = importers.Map{
		...
		"snowflake_main_test": {
			...
			ThirdParty: importers.List{
				...
				`"github.com/asadullahfarooqi/sqlboiler/v4/drivers/sqlboiler-mysql/driver"`,
	...
	col.BasedOnType = importers.Map{
		...
		"types.JSON": {
			ThirdParty: importers.List{`"github.com/asadullahfarooqi/sqlboiler/v4/types"`},
		},
		"types.Decimal": {
			ThirdParty: importers.List{`"github.com/asadullahfarooqi/sqlboiler/v4/types"`},
		},
